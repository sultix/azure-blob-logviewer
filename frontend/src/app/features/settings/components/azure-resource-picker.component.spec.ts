...
describe('AzureResourcePickerComponent', () => {
  ...
  beforeEach(async () => {
    ...
    fixture.componentRef.setInput('storageAccountControl', storageAccountControl);
  ...
  it('binds the passed form controls to the PrimeNG selects', () => {
    ...
    const selects = fixture.debugElement.queryAll(By.directive(Select)).map((debugElement) => {
      return debugElement.componentInstance as Select;
    });

    expect(selects).toHaveLength(3);
    expect(component.subscriptionControlValue.value).toBe(subscription);
    expect(component.storageAccountControlValue.value).toBe(storageAccount);
    expect(component.containerControlValue.value).toBe(container);
  });
  ...
  it('disables dependent selects until their parent selection exists', () => {
    ...
    expect(component.storageAccountControlValue.disabled).toBe(true);
    expect(component.containerControlValue.disabled).toBe(true);

    subscriptionControl.setValue(createSubscription());
    storageAccountControl.enable();
    fixture.detectChanges();

    expect(component.storageAccountControlValue.disabled).toBe(false);
    expect(component.containerControlValue.disabled).toBe(true);
    ...
    expect(component.containerControlValue.disabled).toBe(false);
  });
});
