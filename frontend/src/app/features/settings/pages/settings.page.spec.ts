...
class AppApiServiceStub implements Partial<AppApiService> {
  importConnectionsFile = vi.fn<() => Promise<{ cancelled: boolean; content: string }>>(async () => ({
    cancelled: false,
    content: '[{"id":"conn-2"}]',
  }));
  exportConnectionsFile = vi.fn<(content: string) => Promise<{ cancelled: boolean }>>(async () => ({
    cancelled: false,
  }));
}
...
describe('SettingsPage', () => {
  ...
  it('renders import/export actions and the dashboard management hint instead of a connection list', () => {
    ...
    expect(fixture.nativeElement.textContent).toContain('Import JSON');
    expect(fixture.nativeElement.textContent).toContain('Export JSON');
    expect(fixture.nativeElement.textContent).toContain('Manage connections in the Dashboard');
  ...
  it('disables export when no saved connections exist', () => {
    ...
    const exportButton = getButtonByText(fixture, 'Export JSON');
  ...
  it('imports connections through the native file bridge and shows success feedback', async () => {
    ...
    const importButton = getButtonByText(fixture, 'Import JSON');
    importButton.click();
...
function getButtonByText(fixture: ComponentFixture<SettingsPage>, text: string): HTMLButtonElement {
